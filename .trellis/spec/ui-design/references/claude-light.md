---
ref_id: claude-light
role: 浅色基底参考 (Light Mode Anchor)
source: D:/git有趣/学习项目合集/UI-studio/awesome-design-md/design-md/claude/DESIGN.md
captured_on: 2026-04-26
status: canonical
---

# Claude 浅色参考 (PaperBanana 白天基底)

> **PaperBanana 采用本系统作为浅色模式的视觉锚点。** 暖羊皮纸底 + Anthropic Serif 单一字重 + 赤陶 CTA + ring 阴影体系。
> 在 PaperBanana 中：academic 主题应回归 warm-brown 主色（OKLCH 映射 `oklch(0.56 0.16 40)`），主背景使用 `oklch(0.96 0.01 95)` 对应 `#f5f4ed` Parchment。
> 任何与本文档冲突的实现都需要被修正，而非反过来修改本文档。
