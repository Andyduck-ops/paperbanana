---
ref_id: linear-dark
role: 暗色基底参考 (Dark Mode Anchor)
source: D:/git有趣/学习项目合集/UI-studio/awesome-design-md/design-md/linear.app/DESIGN.md
captured_on: 2026-04-26
status: canonical
---

# Linear 暗色参考 (PaperBanana 夜晚基底)

> **PaperBanana 采用本系统作为暗色模式的视觉锚点。** 近黑画布 + Inter Variable (cv01/ss03) + 510 字重 + 单一靛紫品牌色 + 半透明白边。
> 在 PaperBanana 中：暗色模式的主背景为 `oklch(0.10 0.005 264)` 对应 `#08090a`；面板使用 `oklch(0.13 0.005 264)` 对应 `#0f1011`；品牌色保持靛紫但允许在 academic/japanese-bw 等主题下被覆盖。
> 暗色模式禁止使用暖色调 chrome（borders/backgrounds），只允许内容（图像、引文）保持原色。
