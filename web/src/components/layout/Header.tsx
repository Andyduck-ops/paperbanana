...
export function Header({
  ...
  return (
    <div className="rounded-[1.6rem] border border-border/70 bg-card/90 px-4 py-3 shadow-[var(--theme-surface-shadow)] sm:px-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <a href="/" className="flex items-center gap-3 hover:opacity-80 transition-opacity flex-shrink-0">
          <img src="/images/logo.png" alt="" className="h-10 w-10 rounded-2xl object-cover ring-1 ring-border/60" />
        ...
        <div className="flex flex-col gap-3 lg:items-end">
          <div className="flex flex-wrap items-center gap-2">
            <a
              href="/projects"
              className="flex min-h-10 items-center gap-2 rounded-xl border border-border/70 bg-muted/70 px-3 py-2 text-sm text-foreground hover:bg-muted transition-colors"
            ...
            {onHistoryClick && (
              <button
                ...
                className={`flex min-h-10 items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-colors ${
                  isHistoryOpen ? 'border-primary bg-primary text-primary-foreground' : 'border-border/70 bg-muted/70 text-foreground hover:bg-muted'
                }`}
            ...
            {onSettingsClick && (
              <button
                ...
                className={`flex min-h-10 items-center gap-2 rounded-xl border px-3 py-2 text-sm transition-colors ${
                  isSettingsOpen ? 'border-primary bg-primary text-primary-foreground' : 'border-border/70 bg-muted/70 text-foreground hover:bg-muted'
                }`}
              ...
              <div className="flex items-center gap-1.5">
                {themes.map((themeItem) => (
                  <button
                    ...
                    className={`h-6 w-6 rounded-full border-2 transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1 ${
                      theme === themeItem.id
                    ...
                    title={t(`theme.options.${themeItem.id}.label`) || themeItem.name}
                    aria-label={t(`theme.options.${themeItem.id}.label`) || themeItem.name}
